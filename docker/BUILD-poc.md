...
```bash
# v0.20 (torch 2.9.1 / CUDA 12.9), for A100 + local + H100:
docker buildx build --target vllm-openai \
  ...
  -f docker/Dockerfile \
  -t ghcr.io/axeltec-software/vllm:v0.15-decode-poc-cu129 --load .
```
...
docker build -f docker/Dockerfile.poc-overlay \
  --build-arg BASE=ghcr.io/axeltec-software/vllm:v0.15-decode-poc-cu129 \
  -t ghcr.io/axeltec-software/vllm:v0.15-decode-poc-cu129-r2 .
```
...
docker buildx build -f mlnode/packages/api/Dockerfile \
  --build-arg VLLM_BASE=ghcr.io/axeltec-software/vllm:v0.15-decode-poc-cu129 \
...
docker push ghcr.io/axeltec-software/vllm:v0.15-decode-poc-cu129
