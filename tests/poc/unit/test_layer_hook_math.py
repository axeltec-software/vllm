...
class TestHookTransformation:
    ...
    def test_tuple_output_both_components_transformed(self):
        ...
        # Hook transforms in-place (cudagraph-safe), so capture the pre-transform
        # residual to compare against (input ref == output ref after in-place).
        ...
        assert not torch.allclose(base_residual, residual_out, atol=1e-4), \
