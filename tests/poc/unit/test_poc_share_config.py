...
@pytest.mark.parametrize("share,budget,expected", [
    (0.0, 8192, 0),       # chat only — PoC paused
    (0.5, 8192, 4096),    # even split
    (1.0, 8192, 8192),    # PoC greedy
    (0.25, 1000, 250),
])
def test_poc_token_budget_math(share, budget, expected):
    # Mirrors the scheduler: poc_token_budget = int(poc_share * token_budget).
    assert int(share * budget) == expected
