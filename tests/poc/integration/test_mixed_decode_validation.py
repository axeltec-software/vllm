...
through the Phase-2 step-driven mixed-decode path, which never consumes
``inference_k_points_steps``. The server returned the ``-1`` sentinel for
``n_sphere_mismatches`` instead of the real aligned count, so an honest validator
could not separate honest from fraud once mixed decode became the default.

Fix: ``is_validation`` requests always run the pure ``execute_poc_forward`` path
(which performs the aligned, no-cascade recompute and computes the real
``n_sphere_mismatches``), regardless of the mixed-decode flag — while ordinary
decode-PoC *generation* still mixes with chat.
...
def _validate_roundtrip(url: str) -> dict[int, dict]:
    ...
    # warmup: absorb first-request-after-boot init (lazy hooks/reservation)
    try:
