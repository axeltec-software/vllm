...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        # PoC: poc_max_batch_size == 0 means AUTO -> scale the per-step PoC nonce cap
        # with the machine's own concurrency limit (max_num_seqs), so PoC fills the
        # batch like inference does instead of being pinned to a fixed constant that
        # throttles it on bigger machines. Explicit >0 overrides.
        if self.cache_config is not None and self.scheduler_config is not None:
            ...
            self.cache_config.poc_max_batch_size = resolve_poc_max_batch_size(
                self.cache_config.poc_max_batch_size, self.scheduler_config.max_num_seqs)
