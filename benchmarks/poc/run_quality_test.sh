...
#
# Requires: lm-eval installed in the active venv; server started with --poc-decode.
...
# ---------------------
# PYTHON        python with vllm+lm-eval (default: /tmp/vbuild-proposal/bin/python)
...
# SERVER_ARGS   extra 'vllm serve' flags (default: "--poc-decode --gpu-memory-utilization 0.9 --max-model-len 4096")
...
PYTHON="${PYTHON:-/tmp/vbuild-proposal/bin/python}"
EVAL_SCRIPT="$SCRIPT_DIR/quality_gsm8k.py"

MODEL="${MODEL:-RedHatAI/Qwen2.5-7B-Instruct-quantized.w8a16}"
SERVER_HOST="${SERVER_HOST:-127.0.0.1}"
SERVER_PORT="${SERVER_PORT:-}"
SERVER_ARGS="${SERVER_ARGS:---poc-decode --gpu-memory-utilization 0.9 --max-model-len 4096}"
TASKS="${TASKS:-gsm8k}"
