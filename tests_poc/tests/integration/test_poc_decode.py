...
from tests_poc.utils import poc_request_body
...
class TestDecodeStepStructure:
    ...
    def test_sphere_k_present_with_decode(self, url):
        """sphere_k field is present in each artifact when poc_decode=True."""
        data = _poc_with_decode(url, "0xdecode_sphere_k", [1, 2], max_tokens=3)
        for artifact in data["artifacts"]:
            assert "sphere_k" in artifact, \
                f"Artifact nonce={artifact['nonce']} missing 'sphere_k'"
            assert 0 <= artifact["sphere_k"] < SPHERE_POINTS
